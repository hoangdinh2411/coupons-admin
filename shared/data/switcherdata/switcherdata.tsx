...
export const updateBackgroundColor = (bgColor1: string, bgColor2: string) => {
  const newState = {
    ...
    dataHeaderStyles: localStorage.getItem('rixzoheader')
      ? localStorage.getItem('rixzoheader')
      : 'dark',
...
export const ThemePrimaryColor = () => {
  ...
  const handleInput = (e: { target: { value: any } }) => {
    ...
    const { r, g, b }: any = hexToRgb(color);
...
export const LocalStorageBackup = (setpageloading: (arg0: boolean) => void) => {
  ...
  localStorage.rixzonavstyles === 'menu-click'
    ? updateNavStyle('menu-click', 'menu-click-closed')
    : '';
  localStorage.rixzonavstyles === 'menu-hover'
    ? updateNavStyle('menu-hover', 'menu-hover-closed')
    : '';
  localStorage.rixzonavstyles === 'icon-click'
    ? updateNavStyle('icon-click', 'icon-click-closed')
    : '';
  localStorage.rixzonavstyles === 'icon-hover'
  ...
  if (localStorage.bodyBg) {
    const newState = {
      ...
      dataThemeMode: 'dark',
      dataHeaderStyles: localStorage.rixzoheader
        ? localStorage.rixzoheader
        : localStorage.rixzodarkTheme
          ? 'dark'
          : 'dark',
      ...
      dataMenuStyles: localStorage.rixzomenu
        ? localStorage.rixzomenu
        : localStorage.rixzodarkTheme
          ? 'dark'
          : 'dark',
    };
  ...
  if (localStorage.rixzoverticalstyles) {
    const verticalstyles = localStorage.getItem('rixzoverticalstyles');

    switch (verticalstyles) {
      case 'default':
        const defaultId = switcherDoucmentIdSelector('switcher-default-menu') as HTMLInputElement;
        if (defaultId) {
      ...
      case 'closed':
        const closedId = switcherDoucmentIdSelector('switcher-icontext-menu') as HTMLInputElement;
        if (closedId) {
      ...
      case 'icontext':
        const icontextId = switcherDoucmentIdSelector('switcher-icontext-menu') as HTMLInputElement;
        if (icontextId) {
      ...
      case 'overlay':
        const overlayId = switcherDoucmentIdSelector('switcher-detached') as HTMLInputElement;
        if (overlayId) {
      ...
      case 'detached':
        const detachedId = switcherDoucmentIdSelector('switcher-detached') as HTMLInputElement;
        if (detachedId) {
      ...
      case 'doublemenu':
        const doubleId = switcherDoucmentIdSelector('switcher-double-menu') as HTMLInputElement;
        if (doubleId) {
      ...
      default:
        const defaultbutton = switcherDoucmentIdSelector(
