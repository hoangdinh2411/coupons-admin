...
const Menuloop = ({ MenuItems, level, handleToMenu}: any) => {
  ...
  return (
    <Fragment>
      <Link href="#!" scroll={false} className={`side-menu__item  ${MenuItems?.selected ? "active" : ""}`} onClick={(event) => { event.preventDefault(); handleToMenu(event, MenuItems, undefined, true); }} >
      {/* onMouseEnter={(event) => HoverToggleInnerMenuFn(event, MenuItems)} */}
        <i className="ri-arrow-right-s-line side-menu__angle"></i>
        <OverlayTrigger placement="right" overlay={<Tooltip id="button-tooltip" className='double-menu-tooltip'>{MenuItems.title}</Tooltip>}>
          <div className={`${local_varaiable?.dataVerticalStyle == 'doublemenu' ? '' : 'd-none'}`}>{MenuItems.icon}</div>
        </OverlayTrigger>
        ...
        {MenuItems.children.map((firstlevel: any, index: any) =>
          <li className={`${firstlevel.menutitle ? 'slide__category' : ''} ${firstlevel?.type == 'empty' ? 'slide' : ''} ${firstlevel?.type == 'link' ? 'slide' : ''} ${firstlevel?.type == 'sub' ? 'slide has-sub' : ''} ${firstlevel?.active ? 'open' : ''} ${firstlevel?.selected ? 'active' : ''}`} key={index}>
            ...
            {firstlevel.type === "empty" ?
              <Link href="#!" className={`side-menu__item`} onClick={handleClick}>{firstlevel.title} {firstlevel.icon}</Link>
              : ""}
            {firstlevel.type === "sub" &&
              <Menuloop MenuItems={firstlevel} handleToMenu={handleToMenu} level={level + 1} />}
          </li>
        )}
